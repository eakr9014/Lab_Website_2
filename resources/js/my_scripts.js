...
function loadStatsPage(){
	...
	var wins = 0;
	var losses = 0;
	console.log(tbl.rows[11].cells[2].innerHTML > tbl.rows[11].cells[3].innerHTML);
	for(i=2; i<tbl.rows.length; i++){
		us = tbl.rows[i].cells[2].innerHTML;
		them = tbl.rows[i].cells[3].innerHTML;
		if(parseInt(us)>parseInt(them)){
...
function loadPlayersPage(){
	for(i=0; i< players.length; i++){
		...
		mytag.setAttribute('class', "dropdown-item")
		mytag.setAttribute('href', "#");
		mytag.setAttribute('onclick', `switchPlayers(${i})`);
		mytag.innerText = players[i].name;
		document.getElementById("player_selector").appendChild(mytag);
	}
}
